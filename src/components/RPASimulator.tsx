    ...
    <div className="w-full max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Mengotomatisasi input form ke sistem database utama (tanpa campur tangan manusia).</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-slate-900 rounded-xl p-6 shadow-xl text-slate-300 font-mono text-xs overflow-hidden relative flex flex-col h-[500px]">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar flex flex-col justify-end">
            ...
            <motion.div initial={{opacity:0}} animate={{opacity: 1}} transition={{delay: 0.5}} className="text-emerald-400">
               &gt; Tersambung ke Database Master Siak (V 9.12)
            </motion.div>
            <motion.div initial={{opacity:0}} animate={{opacity: 1}} transition={{delay: 1}} className="text-slate-400">
        ...
        <div className="lg:col-span-2">
           <div className="bg-white rounded-xl shadow-lg border border-slate-200 h-[500px] flex flex-col overflow-hidden">
              <div className="bg-slate-100 px-4 py-3 border-b border-slate-200 flex items-center gap-4">
                 ...
                 <div className="bg-white px-3 py-1 rounded text-xs text-slate-500 shadow-sm flex-1 truncate flex items-center">
                    <Database size={12} className="mr-2" />
                    internal.siak.go.id/master/penduduk/entri-cepat
                 </div>
              </div>

              <div className="p-8 flex-1 overflow-y-auto bg-slate-50 relative">
                 {isComplete && (
                   <motion.div 
                   ...
                   >
                     <div className="bg-white p-6 rounded-2xl shadow-xl border border-emerald-100 text-center max-w-sm">
                        <CheckCircle2 size={64} className="text-emerald-500 mx-auto mb-4" />
                        <h3 className="text-xl font-bold text-slate-800 mb-2">Tersimpan ke Database</h3>
                        <p className="text-sm text-slate-500 mb-6">Proses otomatisasi sukses. Data warga tervalidasi dan diunggah tanpa kesalahan ketik (Zero Human Error).</p>
                        <button 
                          onClick={onReset}
                 ...
                 <div className="max-w-md mx-auto bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                    <h3 className="font-semibold text-slate-800 mb-6 border-b pb-4">Formulir Integrasi Master</h3>
                    
                    <div className="space-y-5">
                       {fields.map((field, idx) => (
                         <div key={idx} className="relative">
                            <label className="block text-xs font-semibold text-slate-500 uppercase mb-1.5">{field.key}</label>
                            <div className={`
                               w-full p-2.5 rounded-md border text-sm font-medium
                               ${currentStep === idx ? 'border-blue-400 ring-2 ring-blue-100 bg-blue-50/30' : 'border-slate-200 bg-slate-50 text-slate-800'}
